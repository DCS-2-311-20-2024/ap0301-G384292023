...
export function makeCBRobot() {
  ...
  const legR = new THREE.Mesh(LegBox, cardboardMaterial);
  legR.position.set(-legSep / 2, legLen / 2, 0);
  cardboardRobot.add(legR);
  const legL = new THREE.Mesh(LegBox, cardboardMaterial);
  legL.position.set(legSep / 2, legLen / 2, 0);
  cardboardRobot.add(legL);

  //  胴体の作成
  const Bodybox = new THREE.BoxGeometry(bodyW, bodyH, bodyD);
  const body1 = new THREE.Mesh(Bodybox, cardboardMaterial);
  body1.children.forEach((child) => {
    child.castShadow = true;
    child.receiveShadow = true;
  });
  body1.position.y = legLen + bodyH / 2;
  cardboardRobot.add(body1);
  //  腕の設定
  const armBox
    = new THREE.BoxGeometry(armW, armLen, armD);
  const armR = new THREE.Mesh(armBox, cardboardMaterial);
  armR.position.set(-bodyW / 2 - armW / 2, legLen + bodyH - armLen / 2, 0);
  cardboardRobot.add(armR);
  const armL = new THREE.Mesh(armBox, cardboardMaterial);
  armL.position.set(bodyW / 2 + armW / 2, legLen + bodyH - armLen / 2, 0);
  cardboardRobot.add(armL);
  //  頭の設定
  const head = new THREE.Group;
  const headBox = new THREE.BoxGeometry(headW, headH, headD);
  const circleGeometry = new THREE.CircleGeometry(eyeRad, seg);
  ...
  head.add(new THREE.Mesh(triangleGeometry2, blackMaterial));
  head.add(new THREE.Mesh(headBox, cardboardMaterial))
  head.children.forEach((child) => {
    child.castShadow = true;
    child.receiveShadow = true;
  });
